...
function draw() {
    ...
    push();    
        ...
        for(var i = 0; i < 12; i++){
            ...
            if(i==6) { text('A', 0, d); }                                 // DRAW NOTE LETTERS
            if(i==7) { text('E', 0, d); }                                 // are larger
            if(i==8) { text('B', 0, d); }                                 //

            textSize(35); fill(0);                                        //
            if(i==0) { text('D#', -10, d-22); text('Eb', 10, d+10); }     // 
            if(i==1) { text('A#', -10, d-22); text('Bb', 10, d+10); }     //
            if(i==9) { text('F#', -10, d-22); text('Gb', 10, d+10); }     //
            if(i==10){ text('C#', -10, d-22); text('Db', 10, d+10); }     // 'sharps / flats'
            ...
            if(visible){
                for(var i = 0; i < 12; i++){
                    ...
                    if(solfege){
                        ...
                        if(i==2) { text('Fa', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/3-PI/60); }
                        if(i==4) { text('So', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/3-PI/60); }
                        if(i==6) { text('La', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/3-PI/60); }
                        if(i==8) { text('Ti', 0, d*1.075); fill('rgba(0, 255, 0, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/6-PI/60); }
                    } else if(sargam){
                        ...
                        if(i==2) { text('Ma', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/3-PI/60); }
                        if(i==4) { text('Pa', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/3-PI/60); }
                        if(i==6) { text('Dha',0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/3-PI/60); }
                        if(i==8) { text('Ni', 0, d*1.075); fill('rgba(0, 255, 0, 1)'); noStroke(); arc(0, 0, d*2.05, d*2.05, step, step+PI/6-PI/60); }
                    } else {
                        if(i==3) { ellipse  (0, d*1.075-10, 30, 30); fill(255); 
                                   text('1', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke();}
                        if(i==5) { text('2', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke();}
                        if(i==7) { text('3', 0, d*1.075); fill('rgba(0, 255, 0, 1)'); noStroke();}
                        if(i==2) { text('4', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke();}
                        if(i==4) { text('5', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke();}
                        if(i==6) { text('6', 0, d*1.075); fill('rgba(0, 0, 255, 1)'); noStroke();}
                        if(i==8) { text('7', 0, d*1.075); fill('rgba(0, 255, 0, 1)'); noStroke();}
                    }
           ...
           /*  rotate(PI/2);
            ...
            if(visible){
                for(var i = 0; i < 12; i++){
                    ...
                    fill(150); noStroke();
                    if(i==0) { rect(0, d, 33, 76); }
                    if(i==1) { rect(0, d, 33, 76); }
                    if(i==9) { rect(0, d, 33, 76); }
                    if(i==10){ rect(0, d, 33, 76); }
